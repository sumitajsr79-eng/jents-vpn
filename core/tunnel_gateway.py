"""
Jents VPN — Universal Self-Healing Zero-Fail Tunnel Gateway
============================================================
Dual-Mode Autonomous Network Privacy Engine:
1. Encrypted DNS-over-HTTPS (Cloudflare + Google DoH)
2. Upstream Relay Chaining with Instant Autonomous Direct Fallback
3. Kernel-level TCP_NODELAY bidirectional streaming
4. Zero SSL Certificate Tampering / Zero Connection Drops
"""
            ...
            if method == "CONNECT":
                ...
                if self.active_node and self.active_node.get("host") and self.active_node.get("port"):
                    try:
                        ...
                        relay.connect((u_host, u_port))
                        relay.sendall(raw_request)
                        ...
                        if resp and b"200" in resp.split(b"\r\n")[0]:
                            ...
                            if header_end < len(resp):
                                client_sock.sendall(resp[header_end:])
                            remote_sock = relay
                            relayed = True
                        else:
                            relay.close()
                ...
                # Attempt 2: Direct Encrypted DoH Fallback (Zero Fail Guarantee)
                if not relayed:
                    resolved_ip = doh_resolver.resolve(host)
                    direct = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                    self._tune(direct)
                    direct.settimeout(6.0)
                    direct.connect((resolved_ip, port))
                    client_sock.sendall(b"HTTP/1.1 200 Connection Established\r\n\r\n")
                    remote_sock = direct
